...
- Basically files are ultimately text files. 
...
## Updating From Remote Repositories (1)
...
## Updating From Remote Repositories (2)
...
You can make `add` files and `commit` changes. 
...
You can use GitHub tags to create [**Digital Object Identifiers**](http://en.wikipedia.org/wiki/Digital_object_identifier).
...
- Play around with the file system from R and the Shell
    ...
    + If not download <https://github.com/christophergandrud/Rep-Res-ExampleProject1>
    and load `Data/MainData.csv` into R.
...
- Make a pull request to another neighour's repo.
