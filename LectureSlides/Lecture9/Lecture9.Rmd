...
Then draw point estimates from the multivariate normal distribution with `mvrnorm`.
...
```{r}
# Find 95% central interval and mean at each fitted value of edu
central <- drawn_sim %>% group_by(fitted_edu) %>%
            summarise(mean_sim = mean(sim_wc),
...
```{r}
ggplot(central, aes(fitted_edu, mean_sim)) +
